...
""")
    ...
    def check_category_duplicate(self, username: str, category: str) -> bool:
        self.cur.execute("SELECT * FROM category WHERE username=? AND category=?", (username, category))
        result = self.cur.fetchone()
        return False if result else True
        
    def add_category(self, username: str, category: str):
        self.cur.execute("CREATE TABLE IF NOT EXISTS category(username TEXT NOT NULL, category TEXT NOT NULL);")
        if self.check_category_duplicate(username, category):
            self.cur.execute("INSERT INTO category(username, category) VALUES (?,?);", (username, category))
            self.conn.commit()
            print(1)
        else:
            print(0)
